    ...
    <div className="min-h-screen bg-gray-50 font-sans text-black flex flex-col">
      ...
      <header className="bg-white shadow-md p-4 sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          >
            <img src="https://ion.or.kr/common/do/thumbnail?index=499994" alt="솔잎지역아동센터 로고" className="rounded-full" />
          ...
          {/* 데스크톱 네비게이션: '홈' 아이콘 제거 및 아이콘 크기 조정 */}
          <nav className="hidden md:flex space-x-6 items-center">
            <NavItem icon={<Info size={30} />} text="센터 소개" onClick={() => navigate('about')} isActive={currentPage === 'about'} />
            <NavItem icon={<BookOpen size={30} />} text="주요 사업" onClick={() => navigate('programs')} isActive={currentPage === 'programs'} />
            <NavItem icon={<Image size={30} />} text="갤러리" onClick={() => navigate('gallery')} isActive={currentPage === 'gallery'} />
            <NavItem icon={<Bell size={30} />} text="공지사항" onClick={() => navigate('announcements')} isActive={currentPage === 'announcements'} />
            <NavItem icon={<Mail size={30} />} text="오시는 길" onClick={() => navigate('contact')} isActive={currentPage === 'contact'} />
            <NavItem icon={<HeartHandshake size={30} />} text="후원/자원봉사" onClick={() => navigate('support')} isActive={currentPage === 'support'} />
          </nav>

          {/* 모바일 메뉴 버튼: `md:hidden`을 사용하여 중간(md) 화면 이상에서는 숨기고 모바일에서만 표시됩니다. */}
          <button
        ...
        {/* 모바일 네비게이션 (토글): '홈' 아이콘 제거 */}
        {isMobileMenuOpen && (
          <nav className="md:hidden mt-4 flex flex-col space-y-2 bg-gray-50 p-4 rounded-lg shadow-inner">
            <NavItem icon={<Info size={20} />} text="센터 소개" onClick={() => navigate('about')} isActive={currentPage === 'about'} isMobile />
            <NavItem icon={<BookOpen size={20} />} text="주요 사업" onClick={() => navigate('programs')} isActive={currentPage === 'programs'} isMobile />
            <NavItem icon={<Image size={20} />} text="갤러리" onClick={() => navigate('gallery')} isActive={currentPage === 'gallery'} isMobile />
            <NavItem icon={<Bell size={20} />} text="공지사항" onClick={() => navigate('announcements')} isActive={currentPage === 'announcements'} isMobile />
            <NavItem icon={<Mail size={20} />} text="오시는 길" onClick={() => navigate('contact')} isActive={currentPage === 'contact'} isMobile />
            <NavItem icon={<HeartHandshake size={20} />} text="후원/자원봉사" onClick={() => navigate('support')} isActive={currentPage === 'support'} isMobile />
          </nav>
        )}
      </header>
...
const NavItem = ({ icon, text, onClick, isActive, isMobile = false }) => (
    ...
    >
      {icon}
      {/* [코드 수정 및 주석 추가]
        데스크톱과 모바일 화면에 따라 글자 크기를 다르게 적용하여 최적화합니다.
        - 데스크톱 (isMobile=false): 'text-sm'으로 설정하여 아이콘과 조화를 이루면서 가독성을 높였습니다.
        - 모바일 (isMobile=true): 'text-base'로 설정하여 터치 편의성과 작은 화면에서의 가독성을 확보했습니다.
      */}
...
const AboutPage = () => (
  <section className="bg-white p-8 rounded-lg shadow-lg animate-fade-in">
    ...
    <div className="mb-8">
      ...
      <ul className="list-disc list-inside text-black space-y-2 pl-4">
        <li>2010년 03월: 솔잎지역아동센터 설립 인가</li>
        <li>2012년 07월: 제1회 솔잎 꿈나무 발표회 개최</li>
        <li>2015년 11월: 환경 개선 사업 완료 (놀이실, 학습실 리모델링)</li>
        <li>2018년 05월: 지역사회 연계 프로그램 '솔잎과 함께하는 문화 탐방' 시작</li>
        <li>2020년 03월: 설립 10주년 기념 행사 개최</li>
        <li>2023년 09월: 아동 심리 상담 프로그램 도입</li>
      </ul>
    </div>
...
const StaffCard = ({ name, role, description, imgSrc }) => (
  <div className="bg-gray-50 p-6 rounded-lg shadow-md flex items-center space-x-4">
    <img
      src={imgSrc}
      alt={name}
      className="w-16 h-16 rounded object-cover border-8 border-red-500"
    ...
    <div className="grid md:grid-cols-2 gap-8 mb-8">
      <div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <p className="text-gray-800">은행: IM뱅크(대구은행)</p>
          <p className="text-gray-800">계좌번호: 033-10-004910</p>
          <p className="text-gray-800">예금주: 솔잎지역아동센터</p>
        </div>
